"""API tests for the Google OAuth device-code sign-in endpoints. The
device-flow HTTP calls themselves are tested in
tests/unit/test_oauth_google.py; these tests stub that module to verify
...
def auth_headers(client):
    client.post(
        "/api/v1/users", json={"email": "google-oauth@example.com", "password": "password123"}
    )
    login = client.post(
        "/api/v1/auth/login",
        json={"email": "google-oauth@example.com", "password": "password123"},
    )
...
def test_start_requires_authentication(client):
    response = client.post("/api/v1/mail-accounts/oauth/google/start")
...
def test_start_returns_503_when_not_configured(client, auth_headers, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: Settings(google_oauth=GoogleOAuthSettings(enabled=False)),
    )

    response = client.post("/api/v1/mail-accounts/oauth/google/start", headers=auth_headers)

    assert response.status_code == 503


def test_start_returns_503_when_client_secret_missing(client, auth_headers, monkeypatch):
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.get_settings",
        lambda: Settings(
            google_oauth=GoogleOAuthSettings(enabled=True, client_id="cid", client_secret="")
        ),
    )
...
def test_start_returns_user_code(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.start_google_device_flow",
        lambda client, user_id: flow,
    )
    ...
    assert body["verification_uri"] == "https://www.google.com/device"


def test_poll_returns_pending(client, auth_headers, monkeypatch):
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.poll_google_device_flow",
        lambda client, flow_id, user_id: False,
    )

    response = client.get("/api/v1/mail-accounts/oauth/google/poll/f1", headers=auth_headers)
...
def test_poll_maps_expired_to_408(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr("app.api.v1.endpoints.mail_accounts.poll_google_device_flow", raise_expired)
...
def test_poll_maps_not_found_to_404(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.poll_google_device_flow", raise_not_found
    )
...
def test_poll_maps_failed_to_400(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr("app.api.v1.endpoints.mail_accounts.poll_google_device_flow", raise_failed)
...
def test_finalize_creates_oauth_account(client, auth_headers, monkeypatch):
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.take_completed_google_tokens",
    ...
    response = client.post(
        "/api/v1/mail-accounts/oauth/google/finalize",
        json={"flow_id": "f1", "email_address": "owner@gmail.com"},
        headers=auth_headers,
    )

    assert response.status_code == 201
    body = response.json()
    assert body["email_address"] == "owner@gmail.com"
    assert body["auth_type"] == "oauth_google"
...
def test_finalize_without_completed_flow_returns_404(client, auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.mail_accounts.take_completed_google_tokens", raise_not_found
    )
...
def test_reconnect_updates_existing_account(client, auth_headers, monkeypatch):
    ...
    response = client.post(
        f"/api/v1/mail-accounts/{account_id}/oauth/google/reconnect",
        json={"flow_id": "f2"},
        headers=auth_headers,
    )
...
def test_reconnect_unknown_account_returns_404(client, auth_headers, monkeypatch):
    ...
    response = client.post(
        "/api/v1/mail-accounts/999999/oauth/google/reconnect",
        json={"flow_id": "f1"},
        headers=auth_headers,
    )
